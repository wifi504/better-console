...
public class Table extends Component<Table> {
    ...
    private LinkedList<HashMap<Integer, Component<?>>> cells; // 单元格数据
     ...
     * @param component 单元格组件
     * @return 可以链式调用
     */
    public Table setCell(int colX, int rowY, Component<?> component) {
        // 判断是否有这么多行，如果没有就在最后进行追加
        if (rowY > cells.size()) appendRows(rowY - cells.size());
        HashMap<Integer, Component<?>> row = cells.get(rowY - 1);
        row.put(colX - 1, component);
     ...
     * @return 可以链式调用
     */
    public Table setCell(int colX, int rowY, String text) {
        return setCell(colX, rowY, cf.newSpanText(text));
    }
     ...
     * @return 可以链式调用
     */
    public Table setCell(Component<?> component) {
        return setCell(pointX, pointY, component);
    }
     ...
     * @return 可以链式调用
     */
    public Table setCell(String text) {
        return setCell(cf.newSpanText(text));
    }
     ...
     * @param align 对齐方式 Table.LEFT Table.CENTER Table.RIGHT
     * @return 可以链式调用
     */
    public Table setAlignment(int colX, int align) {
     ...
     * @return 可以链式调用
     */
    public Table insertRow(int rowY) {
        // 如果指定行不存在，则直接在最后一行追加
     ...
     * @return 可以链式调用
     */
    public Table insertRows(int rowY, int n) {
        for (int i = 0; i < n; i++) {
            insertRow(rowY);
        }
        return this;
     ...
     * @return 可以链式调用
     */
    public Table deleteRow(int rowY) {
        cells.remove(rowY - 1);
        return this;
    }
     ...
     * @return 可以链式调用
     */
    public Table drawLine(int rowY) {
     ...
     * @return 可以链式调用
     */
    public Table drawFullBorder(boolean b) {
        this.isFullBorder = b;
        return this;
    }
     ...
     * @return 可以链式调用
     */
    public Table drawNoneBorder(boolean b) {
        this.isNoneBorder = b;
        return this;
    }
     ...
     * @return 可以链式调用
     */
    public Table transpositional(boolean b) {
    ...
    protected String thisRender() {
        ...
        this.data.put(TEXT, renderTable);
        return super.thisRender();
    }

    /**
     * 更新绑定器
     * 表格组件不支持绑定变量，因为涉及绘图问题
     * 如果有动态数据，自行封装组件作为表格内容
     *
     * @param callback 变量池函数式接口 ref(v -> v.bind(...))
     * @return 可以链式调用
     */
    @Override
    public Table ref(ObjectCallback callback) {
        return this;
    }
}
